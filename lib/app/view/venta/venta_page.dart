...
class _VentaPageState extends State<VentaPage> {
  ...
  Future<void> _cargarProductos() async {
    ...
    try {
      //final apiProducto = ApiServiceProducto(client: http.Client());
  ...
  void agregarItem() {
    if (productoSeleccionado != null &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Ítem agregado a la venta")),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void guardarVenta() async {
    if (items.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("No hay items en la venta")),
      );
      return;
    }
    ...
    print(venta);
    try {
      //final apiVenta = ApiServiceVenta(client: http.Client());
      ...
      setState(() {
        ...
        ventaExitosa = true;
        ventaId = respuesta['ventaId'];
        mensajeResultado = "Venta guardada exitosamente! ID: ${respuesta['ventaId']}";
      });
      ...
      setState(() {
        guardandoVenta = false;
        ventaExitosa = false;
        mensajeResultado = "Error al guardar la venta: $e";
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(mensajeResultado)),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Venta - Cliente: ${widget.cliente['clienteNombre']}"),
      ),
      body: guardandoVenta
          ? _buildLoadingScreen()
          : ventaExitosa
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 20),
          Text("Guardando venta...", style: TextStyle(fontSize: 18)),
        ],
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 10),
          Text("ID de Venta: $ventaId", style: TextStyle(fontSize: 18)),
          SizedBox(height: 30),
          ElevatedButton.icon(
            onPressed: volverAInicio,
    ...
    return Padding(
      ...
      child: Column(
        children: [
          // Info del cliente
          ListTile(
            leading: widget.cliente['img'] != null && widget.cliente['img'] != ''
                ? CircleAvatar(
              backgroundImage: NetworkImage(widget.cliente['img']),
            )
                : CircleAvatar(child: Icon(Icons.person)),
            title: Text(widget.cliente['clienteNombre']),
            subtitle: Text(widget.cliente['clienteIdentificacion']),
          ),
          Divider(),
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  title:
                  Text("${item['producto']} - Cant: ${item['cantidad']}"),
                  subtitle: Text(
